...
and dependencies formulas f =
  ...
  match f  with
    ...
    let inf,sup = coords c r, coords (c+length) (r+width)  in
...
and mark_cell_as_evaluated coord =
  Hashtbl.replace marks coord White
...
let init () =
  ...
  let l =
    [cell ~v:1 (int 1); cell ~v:2 (int 2);cell ~v:3 (int 3); cell (count 0 0 4 4 1);
     cell ~v:1 (int 1); cell ~v:2 (int 2);cell ~v:3 (int 3); cell (count 0 1 3 1 2);
     cell ~v:1 (int 1); cell ~v:2 (int 2);cell ~v:3 (int 3); cell (count 0 2 3 1 3);
     cell ~v:1 (int 1); cell ~v:2 (int 2);cell ~v:3 (int 3); cell (count 0 3 3 1 4);
    ] in
